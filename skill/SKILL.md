...
- **Named-collection values are snapshotted at stream-create time.** `ALTER NAMED COLLECTION` updates `system.named_collections`, but an *existing* external stream keeps using the value it captured at `CREATE EXTERNAL STREAM` time — proton calls `updateSettingsByNamedCollection` only during storage construction. **`ALTER STREAM` is rejected by the Python external-stream engine** (`Cannot alter settings…`), so to rotate credentials you must `DROP STREAM` and re-create. Plan for that in your install/upgrade flow.
